---
title: "27-Black-youth-missing-graphics-calculations"
output: html_notebook
---

The purpose of this notebook is to create a data frame that we can use to plot the Black youth missing graphic from Figures 16-22 on Pages 115-118 of the Access Denied Report.We will call the final data frame "BL_missing_plot_df."
...
We will use the following code to find percent missingness in Black students

```{r}
# Get percent of students in GT in each state
BL_state_totals_perc <- ocr_17_wide %>%
  group_by(LEA_STATE) %>%
  summarise(tot_BL_enr = (sum(SCH_ENR_BL_M,na.rm = TRUE) + sum(SCH_ENR_BL_F,na.rm = TRUE)), tot_BL_gt_enr = (sum(SCH_GTENR_BL_M,na.rm = TRUE) + sum(SCH_GTENR_BL_F,na.rm = TRUE)), state_BL_perc_gt = (sum(SCH_GTENR_BL_M,na.rm = TRUE) + sum(SCH_GTENR_BL_F,na.rm = TRUE))/(sum(SCH_ENR_BL_M,na.rm = TRUE) + sum(SCH_ENR_BL_F,na.rm = TRUE))) %>%
  ungroup()
```

```{r}
# Add percent of students in GT in each state to main DF
BL_missing_df <- state_missing_totals_df %>%
  left_join(BL_state_totals_perc, by = c("LEA_STATE" = "LEA_STATE"))
```
...
BL_enrollment_no_gt_df <- ocr_17_wide %>%
  filter(SCH_GT_IND == "No") %>%
  group_by(LEA_STATE) %>%
  summarise(tot_BL_enr_no_gt = (sum(SCH_ENR_BL_M,na.rm = TRUE) + sum(SCH_ENR_BL_F,na.rm = TRUE))) %>%
  ungroup()
...
BL_enrollment_w_gt_df <- ocr_17_wide %>%
  filter(SCH_GT_IND == "Yes") %>%
  group_by(LEA_STATE) %>%
  summarise(tot_BL_enr_w_gt = sum(SCH_ENR_BL_M,na.rm = TRUE) + sum(SCH_ENR_BL_F,na.rm = TRUE)) %>%
  ungroup()
```

```{r}
# Add number of students from schools with different service types to full DF
BL_missing_df <- BL_missing_df %>%
  left_join(BL_enrollment_no_gt_df, by = c("LEA_STATE" = "LEA_STATE")) %>%
  left_join(BL_enrollment_w_gt_df, by = c("LEA_STATE" = "LEA_STATE"))
...
BL_missing_df <- BL_missing_df %>%
  mutate(BL_missing_no_GT_lower = round(tot_BL_enr_no_gt * state_perc_gt), BL_missing_no_GT_upper = round(tot_BL_enr_no_gt * state_nonT1_perc_gt))
```

```{r}
#Find number of underrepresented students from gifted education
BL_missing_df <- BL_missing_df %>%
  mutate(BL_missing_underrep_lower = (round((tot_BL_enr_w_gt * state_perc_gt)) - tot_BL_gt_enr), BL_missing_underrep_upper = (round(tot_BL_enr_w_gt * state_nonT1_perc_gt) - tot_BL_gt_enr))
```
...
#Change negative under representation values to zeros
 BL_missing_df[,15:16][BL_missing_df[,15:16] < 0] <- 0
```
...
BL_missing_df <- BL_missing_df %>%
  mutate(BL_perc_missing_lower = ((BL_missing_no_GT_lower + BL_missing_underrep_lower)/(tot_BL_gt_enr + BL_missing_no_GT_lower + BL_missing_underrep_lower)), BL_perc_missing_upper = ((BL_missing_no_GT_upper + BL_missing_underrep_upper)/(tot_BL_gt_enr + BL_missing_no_GT_upper + BL_missing_underrep_upper)))
```
...
BL_plot_fix <- BL_missing_df %>%
  select(LEA_STATE, BL_perc_missing_lower, BL_perc_missing_upper)
```

```{r}
# Find states where lower bound is greater than the upper bound
BL_plot_fix %>%
  filter(BL_perc_missing_lower > BL_perc_missing_upper)
```
...
BL_plot_fix <- BL_plot_fix %>%
  mutate(upperbound_add_on = abs(BL_perc_missing_upper - BL_perc_missing_lower))%>%
  rowwise()%>%
  mutate(bottom_bar = min(BL_perc_missing_upper, BL_perc_missing_lower), top_bar = max(BL_perc_missing_upper, BL_perc_missing_lower))
```

```{r}
# Create a df with only the values we need to plot the lower bounds
BL_lower_plot_df <- BL_plot_fix %>%
  select(LEA_STATE, bottom_bar)

BL_lower_plot_df$bound <- "Lower Bound"

colnames(BL_lower_plot_df) <- c("LEA_STATE", "percent_missing", "bound")
```

```{r}
# Create a df with only the values needed to plot the upper bounds
BL_upper_plot_df <- BL_plot_fix %>%
  select(LEA_STATE, upperbound_add_on)

BL_upper_plot_df$bound <- "Upper Bound"

colnames(BL_upper_plot_df) <- c("LEA_STATE", "percent_missing", "bound")
```

```{r}
# Stack upper and lower data frames on top of each other to get our final data frame for plotting. 
BL_missing_plot_df <- rbind(BL_upper_plot_df, BL_lower_plot_df)
```
