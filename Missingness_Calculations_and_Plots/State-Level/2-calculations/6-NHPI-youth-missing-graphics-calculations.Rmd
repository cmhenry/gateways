---
title: "27-NHPI-youth-missing-graphics-calculations"
output: html_notebook
---

The purpose of this notebook is to create a data frame that we can use to plot the NHPI youth missing graphic from Figures 16-22 on Pages 115-118 of the Access Denied Report.We will call the final data frame "HP_missing_plot_df."
...
We will use the following code to find percent missingness in NHPI students

```{r}
# Get percent of students in GT in each state
HP_state_totals_perc <- ocr_17_wide %>%
  group_by(LEA_STATE) %>%
  summarise(tot_HP_enr = (sum(SCH_ENR_HP_M,na.rm = TRUE) + sum(SCH_ENR_HP_F,na.rm = TRUE)), tot_HP_gt_enr = (sum(SCH_GTENR_HP_M,na.rm = TRUE) + sum(SCH_GTENR_HP_F,na.rm = TRUE)), state_HP_perc_gt = (sum(SCH_GTENR_HP_M,na.rm = TRUE) + sum(SCH_GTENR_HP_F,na.rm = TRUE))/(sum(SCH_ENR_HP_M,na.rm = TRUE) + sum(SCH_ENR_HP_F,na.rm = TRUE))) %>%
  ungroup()
```

```{r}
# Add percent of students in GT in each state to main DF
HP_missing_df <- state_missing_totals_df %>%
  left_join(HP_state_totals_perc, by = c("LEA_STATE" = "LEA_STATE"))
```
...
HP_enrollment_no_gt_df <- ocr_17_wide %>%
  filter(SCH_GT_IND == "No") %>%
  group_by(LEA_STATE) %>%
  summarise(tot_HP_enr_no_gt = (sum(SCH_ENR_HP_M,na.rm = TRUE) + sum(SCH_ENR_HP_F,na.rm = TRUE))) %>%
  ungroup()
...
HP_enrollment_w_gt_df <- ocr_17_wide %>%
  filter(SCH_GT_IND == "Yes") %>%
  group_by(LEA_STATE) %>%
  summarise(tot_HP_enr_w_gt = sum(SCH_ENR_HP_M,na.rm = TRUE) + sum(SCH_ENR_HP_F,na.rm = TRUE)) %>%
  ungroup()
```
...
HP_missing_df <- HP_missing_df %>%
  left_join(HP_enrollment_no_gt_df, by = c("LEA_STATE" = "LEA_STATE")) %>%
  left_join(HP_enrollment_w_gt_df, by = c("LEA_STATE" = "LEA_STATE"))
...
HP_missing_df <- HP_missing_df %>%
  mutate(HP_missing_no_GT_lower = round(tot_HP_enr_no_gt * state_perc_gt), HP_missing_no_GT_upper = round(tot_HP_enr_no_gt * state_nonT1_perc_gt))
```

```{r}
#Find number of underrepresented students from gifted education
HP_missing_df <- HP_missing_df %>%
  mutate(HP_missing_underrep_lower = (round((tot_HP_enr_w_gt * state_perc_gt)) - tot_HP_gt_enr), HP_missing_underrep_upper = (round(tot_HP_enr_w_gt * state_nonT1_perc_gt) - tot_HP_gt_enr))
```
...
#Change negative under representation values to zeros
 HP_missing_df[,15:16][HP_missing_df[,15:16] < 0] <- 0
```
...
HP_missing_df <- HP_missing_df %>%
  mutate(HP_perc_missing_lower = ((HP_missing_no_GT_lower + HP_missing_underrep_lower)/(tot_HP_gt_enr + HP_missing_no_GT_lower + HP_missing_underrep_lower)), HP_perc_missing_upper = ((HP_missing_no_GT_upper + HP_missing_underrep_upper)/(tot_HP_gt_enr + HP_missing_no_GT_upper + HP_missing_underrep_upper)))
```
...
HP_plot_fix <- HP_missing_df %>%
  select(LEA_STATE, HP_perc_missing_lower, HP_perc_missing_upper)
```

```{r}
# Find states where lower bound is greater than the upper bound
HP_plot_fix %>%
  filter(HP_perc_missing_lower > HP_perc_missing_upper)
```
...
#HP_plot_fix[2,3] <- HP_plot_fix[2,2]

#Change missingness percentage to 100 in states that have no gifted representation at all
HP_plot_fix[8,2:3] <- 1
HP_plot_fix[40,2:3] <- 1
HP_plot_fix[41,2:3] <- 1
HP_plot_fix[48,2:3] <- 1
```
...
HP_plot_fix <- HP_plot_fix %>%
  mutate(upperbound_add_on = abs(HP_perc_missing_upper - HP_perc_missing_lower))%>%
  rowwise()%>%
  mutate(bottom_bar = min(HP_perc_missing_upper, HP_perc_missing_lower), top_bar = max(HP_perc_missing_upper, HP_perc_missing_lower))
```

```{r}
# Create a df with only the values we need to plot the lower bounds
HP_lower_plot_df <- HP_plot_fix %>%
  select(LEA_STATE, bottom_bar)

HP_lower_plot_df$bound <- "Lower Bound"

colnames(HP_lower_plot_df) <- c("LEA_STATE", "percent_missing", "bound")
```

```{r}
# Create a df with only the values needed to plot the upper bounds
HP_upper_plot_df <- HP_plot_fix %>%
  select(LEA_STATE, upperbound_add_on)

HP_upper_plot_df$bound <- "Upper Bound"

colnames(HP_upper_plot_df) <- c("LEA_STATE", "percent_missing", "bound")
```

```{r}
# Stack upper and lower data frames on top of each other to get our final data frame for plotting. 
HP_missing_plot_df <- rbind(HP_upper_plot_df, HP_lower_plot_df)
```
